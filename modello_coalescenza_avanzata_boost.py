...
#analyzed to identify deuteron candidates with the Advanced Coalescence Model (boost corrected).
...
df_pn = root_pandas.read_root('pn_U1S_recap.root', key='pn_U1S_recap_tree')
df_pn.tail(5)
...
pairs = np.delete(pairs, [0,1,2,3,4,5], axis=1) #Delete negligible columns to improve readability
...
df.rename(columns={df.columns[3]:  "pt_p+" }, inplace = True)
df.rename(columns={df.columns[4]:  "p_p+"}, inplace = True)
df.rename(columns={df.columns[5]:  "E_p+" }, inplace = True)
df.rename(columns={df.columns[6]:  "px_n0"}, inplace = True)
df.rename(columns={df.columns[7]:  "py_n0"}, inplace = True)
df.rename(columns={df.columns[8]:  "pz_n0"}, inplace = True)
df.rename(columns={df.columns[9]:  "pt_n0" }, inplace = True)
df.rename(columns={df.columns[10]: "p_n0"}, inplace = True)
df.rename(columns={df.columns[11]: "E_n0" }, inplace = True)
...
##############################################################################################################
#Function definition: Given a certain four-vector, it operates the LT along a certain direction defined by the
...
def gauss_one(q, sigma, theta, beta):
    ...
    #Calculate the perpendicular component of sigma and then the C factor considering the boost
    sigma_perpend = sigma / np.sqrt(pow(np.cos(theta), 2) + pow(gamma*np.sin(theta), 2))
    
    C = (d2 / (d2 + 4*pow(sigma_perpend, 2))) * np.sqrt(d2 / (d2 + 4*sigma2))
...
def gauss_two_phi0(q, sigma, theta, beta):
    ...
    #Calculate the perpendicular component of sigma and then the C factors considering the boost
    sigma_perpend = sigma / np.sqrt(pow(np.cos(theta), 2) + pow(gamma*np.sin(theta), 2))
    
    C_1 = (d2_1 / (d2_1 + 4*pow(sigma_perpend, 2))) * np.sqrt(d2_1 / (d2_1 + 4*sigma2))
    C_2 = (d2_2 / (d2_2 + 4*pow(sigma_perpend, 2))) * np.sqrt(d2_2 / (d2_2 + 4*sigma2))

    #Calculate the probability to form deuteron
    w = 3 * (C_1*delta*np.exp(-q2*d2_1) + C_2*(1-delta)*np.exp(-q2*d2_2))
    
    return w


def gauss_two_r3(q, sigma, theta, beta):
...
sigma = 9.04 #GeV^-1 (Sigma: fenomenologocal parameter rapresenting the size of the formation region)
...
for i in range(0, len(event_array)):
    ...
    for j in range(0,n):
        ...
        q, beta, proton, neutron = CoM_system(proton, neutron) #boost pairs in the CoM, calculate q and beta
        ...
        if q < 0.5:   #For the gauss_two cases: we take into consideration only pairs satisfing this condition
            ...
            p_parall_p = np.sqrt(pow(pair.loc['p_p+'], 2) - pow(pair.loc['pt_p+'], 2))
            p_parall_n = np.sqrt(pow(pair.loc['p_n0'], 2) - pow(pair.loc['pt_n0'], 2))
            
            #Angle between the direction of motion of the pair in the CoM and the z-axis in the original frame
            theta = (pair.loc['pt_p+'] + pair.loc['pt_n0']) / (p_parall_p + p_parall_n)

            w = gauss_two_phi0(q, sigma, theta, beta) #Probabilty that this pair can form a deuteron
...
#np.savetxt('p_advance_U1S_boost_babar.dat', p_deuter)
